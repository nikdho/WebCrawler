...
public class Graph<T> {
 ...
 public Graph(T[] keys) {
     for (int i = 0; i < keys.length; ++i)
         this.addVertex(new GraphNode<T>(keys[i]));
 }
 ...
 public GraphNode<T> BFS(GraphNode<T> startV, T searchKey) {
	 Queue<GraphNode<T>> faj = new Queue<GraphNode<T>>();
	 ArrayList<GraphNode<T>> discovered = new ArrayList<GraphNode<T>>();
	...
	if(v.getKey().equals(searchKey))
		return v;
 	ArrayList<GraphNode<T>> newOnes = new ArrayList<GraphNode<T>>();
...
public String toStringWithWeights() {
 ...
 public static void main(String []args) {
 	Integer[] test = {3, 121, 90};
 	Graph<Integer> testGraph = new Graph<Integer>(test);
